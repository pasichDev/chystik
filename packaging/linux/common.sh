...
build_binaries() {
    ...
    local gui_binary="$target_dir/release/chystik-gui"
    local cli_binary="$target_dir/release/chystik"
    if [[ "${PACKAGING_SKIP_BUILD:-0}" == '1' ]]; then
        test -x "$gui_binary" && test -x "$cli_binary" || {
            echo "PACKAGING_SKIP_BUILD=1 requires $gui_binary and $cli_binary" >&2
            return 1
        }
        return
    fi
    cargo build --manifest-path "$REPO_ROOT/Cargo.toml" --release \
        --package chystik-gui --package chystik-cli --locked
}
...
cli_binary_path() {
    printf '%s\n' "${CARGO_TARGET_DIR:-$REPO_ROOT/target}/release/chystik"
}

cli_manpage_path() {
    local target_dir="${CARGO_TARGET_DIR:-$REPO_ROOT/target}"
    local manpage
    manpage="$(find "$target_dir/release/build" -path '*/out/chystik.1' -type f -print -quit)"
    [[ -n "$manpage" ]] || {
        echo 'CLI man page was not generated by cargo build' >&2
        return 1
    }
    printf '%s\n' "$manpage"
}
...
stage_linux_root() {
    ...
    install -Dm755 "$(cli_binary_path)" "$destination/usr/bin/chystik"
    install -Dm644 "$(cli_manpage_path)" "$destination/usr/share/man/man1/chystik.1"
    mkdir -p \
        ...
        "$destination/usr/share/powershell/Modules/Chystik"
    "$(cli_binary_path)" completion bash > "$destination/usr/share/bash-completion/completions/chystik"
    "$(cli_binary_path)" completion zsh > "$destination/usr/share/zsh/site-functions/_chystik"
    "$(cli_binary_path)" completion fish > "$destination/usr/share/fish/vendor_completions.d/chystik.fish"
    "$(cli_binary_path)" completion powershell > "$destination/usr/share/powershell/Modules/Chystik/chystik.ps1"
